...
var featureStyleList = {
	...
	'おなかま保育室': { color: '#CC00FF', img: 'image/018.png'},
	'川崎認定保育園': { color: '#00D0FF', img: 'image/018.png'},
	'地域保育園': { color: '#999999', img: 'image/018.png'}
};
 ...
 * @return {[type]}            [description]
 */
var onakamaStyleFunction = function(feature, resolution)
{
	var facilityTypeName = feature.get('種別') ? feature.get('種別') : feature.get('Type');
	var style = [];
	if(facilityTypeName === "おなかま保育室") {
		featureStyle = featureStyleList[facilityTypeName];
		style        = nurseryStyleFunction(feature, resolution, featureStyle);
	}
	return style;
 ...
 * @return {[type]}            [description]
 */
var kawasakiNinteiStyleFunction = function(feature, resolution)
{
	var facilityTypeName = feature.get('種別') ? feature.get('種別') : feature.get('Type');
	var style = [];
	if(facilityTypeName === "川崎認定保育園") {
		featureStyle = featureStyleList[facilityTypeName];
		style        = nurseryStyleFunction(feature, resolution, featureStyle);
	}
	return style;
 ...
 * @return {[type]}            [description]
 */
var chiikiStyleFunction = function(feature, resolution)
{
	var facilityTypeName = feature.get('種別') ? feature.get('種別') : feature.get('Type');
	var style = [];
	if(facilityTypeName === "地域保育園") {
		featureStyle = featureStyleList[facilityTypeName];
		style        = nurseryStyleFunction(feature, resolution, featureStyle);
	}
	return style;
...
function baseSchoolStyle(mojicolor, fillcolor) {
	return function(feature, resolution) {
		...
		resolution = Math.floor(resolution * 1000);
		var text = "";
		if(feature.get('P29_005') !== null) {
			text = resolution < 12000 ? feature.get('P29_005') : '';
		}
		if (text !== "") {
			style.push(
					new ol.style.Style({
